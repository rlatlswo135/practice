...
// class View{ //최대한 공통인자를 뽑아보자
//     template:string
//     container:HTMLElement //원래 또는 null인데 null이못오게 밑에서 If로 분기했으니까
//     htmlArray:string[]
//     renderTemplate:string
//     constructor(template:string,containerId:string){
//         const containerElement = document.getElementById(containerId)
//         if(!containerElement){
//             throw `can't find root container`
//         }
...
//         this.htmlArray = [];
//     }
//     updateView():void{ //return이없을때 void라는 타입을쓴다
//         this.container.innerHTML = this.renderTemplate;
//         this.renderTemplate = this.template;
//     }

//     addHtml(htmlString:string){
//         this.htmlArray.push(htmlString)
//     }

//     getHtml():string{
//         const resultHtmlString = this.htmlArray.join('');
//         this.clearHtmlArray();
...
//         </div>
//         `
//         super(template,containerId); //상위클래스에서 상속받으면 반드시 부모클래스를 상속하는 super()키워드를써야 부모클래스에 진입가능함
//         const RESULT_NEWS_URL = NEWS_URL.replace('@currentPage',String(store.currentPage)) //경고의 이유는 replace는 string을 연산하는 함수니까 인자를 string으로 줫으면 좋겟다라고 권고하는것, 그래서 String으로 감싼거지
//         this.api = new NewsFeedApi(RESULT_NEWS_URL)
//         this.feeds = store.readedNews; //재할당해야하니 let으로 (const x)
//         /* 
...
//        if(this.feeds.length === 0){ //최초 호출시에는 ajax를 통해 데이터를 받아 와야겟지? => 페이징때마다 원래 들렷던 페이지까지 또 오청해서 데이터를 받아오는구조라 아예 받아왓으면 배열에 넣어서 요청하고 그런 시간을 줄이는거지
//         this.feeds = store.readedNews = this.api.getData()
//         this.isReadNews()   
...
//             this.setTemplateData('pre_page',String(store.currentPage === 1 ? store.currentPage : store.currentPage-1))
//             //이전페이지 페이징시 1->0으로가는걸 막기위한 방어코드 + 다음페이지도 마찬가지(확인해보니 10페이지밖에 못불러옴)
//             this.setTemplateData('next_page',String(store.currentPage === 10 ? store.currentPage : store.currentPage +1))
//             /*
...
//     this.newsId = location.hash.split('show')[1].substring(1)
//     const RESULT_CONTENT_URL = CONTENT_URL.replace('@id',this.newsId) //위가 내가짠거 (아래가훨씬직관적이고 좋음)
//     this.api = new NewsDetailApi(RESULT_CONTENT_URL)
    
    
// }
// render(){
//         const newsContent = this.api.getData()
...
//             }
//         }
//         const resultCommentList = this.makeComment(newsContent.comments)
    
//         this.setTemplateData('news_comment',resultCommentList)
//         this.setTemplateData('current_page',String(store.currentPage))
//         this.setTemplateData('news_title',newsContent.title)
//         this.setTemplateData('news_content',newsContent.content)
...
//     }
//     makeComment(commentArray:NewsComments[],called = 0):string{ 
...
//                 //재귀 잘이해해보자
//                 commentList.push(this.makeComment(comment.comments,called + 1))
